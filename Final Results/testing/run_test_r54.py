#!/usr/bin/env python3
"""
R54 Domain-Randomisation Test — 20-Obstacle Testing Environment
================================================================
Loads the best R54 policy (r54_best.pth) and runs a single episode
in the 20-dynamic-obstacle tunnel (not used during training).
Retries up to MAX_ATTEMPTS until a SUCCESS is recorded.

Pre-requisite: Gazebo must already be running with the 20-obstacle world
  ros2 launch uav_simulator tunnel_drl.launch.py gui:=true \
      world:=.../tunnel_100m_dynamic_20.world

Usage:
  cd /home/makhosazana/Project/CERLAB-UAV-Autonomy
  source /opt/ros/humble/setup.bash && source install/setup.bash
  python3 "Final Results/testing/run_test_r54.py"
"""
...
def run_episode(node: DroneNode, actor: Actor, attempt: int, noise_sd: float,
    ...
    csv_path = OUT_DIR / f"test_r54_attempt{attempt}_{ts}.csv"
...
def main():
    print("\n" + "="*60)
    print("  R54 BEST POLICY — DOMAIN RANDOMISATION TEST")
    print("  Testing environment: tunnel_100m_dynamic_20.world")
    ...
    # Deterministic spawn offsets — all negative to avoid lbox at X=17, Y=[0,1]
    # Larger spread increases chance of threading past the person at X≈51m
    ...
    # ── Save JSON summary ──────────────────────────────────────────────────────
    ts = time.strftime("%Y%m%d_%H%M%S")
    summary_path = OUT_DIR / f"test_r54_summary_{ts}.json"
    meta = {
        "test_name":         "R54 Best Policy — 20-Obstacle Domain Randomisation Test",
        "testing_world":     "tunnel_100m_dynamic_20.world",
